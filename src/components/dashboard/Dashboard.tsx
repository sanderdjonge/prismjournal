            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div className="glass-card border-border-color bg-surface-elevated backdrop-blur-xl rounded-2xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                        ...
                        <MetricRow label="Profit Factor" value={stats.profitFactor > 0 ? fmtDecimals(stats.profitFactor, 2) : '—'} />
                        <MetricRow label="Avg R-Multiple" value={`${stats.avgRMultiple >= 0 ? '+' : ''}${fmtDecimals(stats.avgRMultiple, 2)}R`} variant={stats.avgRMultiple >= 0 ? 'profit' : 'loss'} />
                        <MetricRow label="Expectancy" value={`${stats.expectancy >= 0 ? '+' : '-'}${symbol}${fmtDecimals(Math.abs(stats.expectancy), 2)}`} variant={stats.expectancy >= 0 ? 'profit' : 'loss'} />
