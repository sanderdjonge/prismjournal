...
export default function BEMetricsWidget({ accountId }: Props) {
    ...
    if (isLoading) {
        return (
            <div className="glass-card border-border-color bg-surface-elevated backdrop-blur-xl rounded-2xl p-6 animate-pulse">
    ...
    if (isError || !data || data.tradeCount === 0) {
        return (
            <div className="glass-card border-border-color bg-surface-elevated backdrop-blur-xl rounded-2xl p-6 space-y-5">
                ...
                <div className="flex items-start gap-2 bg-surface-elevated border border-border-subtle rounded-xl p-4">
                    <ShieldCheck size={12} className="text-text-muted mt-0.5 shrink-0" />
    ...
    return (
        <div className="glass-card border-border-color bg-surface-elevated backdrop-blur-xl rounded-2xl p-6 space-y-5">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-sm font-semibold text-gray-100">SL Management & Breakeven</h3>
                    <p className="text-xs text-text-muted">{tradeCount} trades with initial SL data</p>
                </div>
                <ShieldCheck size={14} className="text-text-muted" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {/* BE Protection Rate */}
                <div className="bg-black/30 rounded-xl p-4 border border-border-subtle space-y-1">
                    <p className="text-[8px] font-black uppercase tracking-widest text-text-muted">BE Protection Rate</p>
                    <p className="text-2xl font-black tracking-tighter" style={{ color: beRateColor }}>
                        {pct(beProtectionRate)}
                    </p>
                    <p className="text-[8px] text-text-muted font-medium leading-tight">moved SL to breakeven</p>
                </div>

                {/* BE Stop-Out Rate */}
                <div className="bg-black/30 rounded-xl p-4 border border-border-subtle space-y-1">
                    <p className="text-[8px] font-black uppercase tracking-widest text-text-muted">BE Stop-Out Rate</p>
                    <p className="text-2xl font-black tracking-tighter" style={{ color: beStopColor }}>
                        {pct(beStopOutRate)}
                    </p>
                    <p className="text-[8px] text-text-muted font-medium leading-tight">BE trades closed at loss</p>
                </div>

                {/* R stats */}
                <div className="bg-black/30 rounded-xl p-4 border border-border-subtle space-y-1">
                    <p className="text-[8px] font-black uppercase tracking-widest text-text-muted">Avg R Captured</p>
                    <p className="text-2xl font-black tracking-tighter" style={{ color: effColor }}>
                        {fmtDecimals(avgRCaptured, 2)}R
                    </p>
                    {avgRPotential > 0 && (
                        <p className="text-[8px] text-text-muted font-medium leading-tight">
            ...
            {avgRPotential > 0 && (
                <div className="space-y-2">
                    ...
                    <p className="text-[8px] text-text-muted font-medium">
